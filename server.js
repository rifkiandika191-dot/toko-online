...
// ---------- Orders ----------
const ORDER_STATUSES = ['baru', 'diproses', 'dikirim', 'selesai', 'batal'];

// Publik: buat pesanan (dipanggil saat checkout dari etalase)
app.post('/api/orders', async (req, res) => {
  ...
  if (!rawItems.length) return res.status(400).json({ error: 'Keranjang kosong' });

  // Hitung total di server (harga produk diambil dari DB bila id ada, anti-manipulasi).
  let total = 0;
  const items = [];
  for (const it of rawItems) {
    const qty = Math.max(1, parseInt(it.qty, 10) || 1);
    let price = Math.max(0, parseInt(it.price, 10) || 0);
    let pname = String(it.name || 'Produk').trim();
    let image = it.image ? String(it.image) : null;
    const id = parseInt(it.id, 10);
    if (id) {
      try {
        const { rows } = await pool.query(
          'SELECT name, price, images FROM products WHERE id=$1', [id],
        );
        if (rows.length) {
          price = rows[0].price;
          pname = rows[0].name;
          if (Array.isArray(rows[0].images) && rows[0].images[0]) image = rows[0].images[0];
        }
      } catch { /* pakai nilai dari klien */ }
    }
    total += price * qty;
    items.push({ id: id || null, name: pname, price, qty, size: it.size || null, image });
  }
  ...
  try {
    ...
    res.status(201).json({ id: rows[0].id, total: rows[0].total, items });
  } catch (e) {
    console.error('[POST /api/orders]', e.message);
